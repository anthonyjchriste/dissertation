...
The ``Payload" \ref{table:Payload} vairiant of message contains data payloads cast to 64-bit precision floating point numbers. This allows us to work with data of multiple type by assuming all data payloads can be cast to 64-bit floats. This message also contains metadata relating to where the data came from, timing information, and a type safe enumeration describing the type of data stored in the payload.
...
The ``MakaiEvent" \ref{table:MakaiEvent} vaiant is a message which includes the ``event\_id" of a newly created event from Makai. This message is received by the ``MakaiEventPlugin" and is used to begin processing new events within Mauka generated by Makai.
...
The ``Measurement" \ref{table:Measurement} variant is a message that contains an instaneous measurement which includes a timestamp, Voltage, Frequency, and THD metrics.
...
The ``GcTrigger" \ref{table:GcTrigger} variant is a message that is sent to the GC plugin to notify it that GC should be performed on the domains specidied in the message.
...
A ``GcUpdate" \ref{table:GcUpdate} variant is a message that informs the GC plugin to update the TTL for a particular document and all documents living udner it. 
...
A ``GcStat" \ref{table:GcStat} variant message is a message that contains statsitcs on the number of items garbage collected from a particular GcDomain.
