...
Two approaches were taken to find similar groups of Frequency incidents with varying success. K-means clustering was used to find groups of related clusters with an empirically found $k$ value. I also attempted to utilize hierarchical clustering, but had issues due to the high memory requirements required for agglomerative clustering. In agglomerative clustering, point-wise distances are found which ends up providing a memory overhead of $O(N^2)$. This memory overhead is too large to be useful on our cloud based OPQ server. I tried to perform agglomerative clustering on a development machine with 32 GB of memory, but still ran into memory issues. Because of these memory issues, I attempted to focus my efforts on k-means which provides memory overhead of $O(N + k * d)$.
...
The above Table displays some interesting features. First, we can gather that 172,864 Frequency sag Incidents all have mean durations less than half a second. Next, we can observe that cluster 5 captures the outlying short duration Voltage sags that have larger deviations from nominal. The rest of the clusters show mean Frequency sag values near 59.75 Hz.
...
Future work will examine extending this concept to other features collected by OPQ and examine including higher dimensional features sets to compare between. Future work will also examine how the categories defined above might be refined to provide more valuable context.
