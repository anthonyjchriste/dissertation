...
Perhaps the THD threshold is a bit too aggressive for Event generation and future work should look at relaxing this threshold a bit and obs how that affects the results.
...
This time, THD is not the cause of the large difference in the comparison. Instead, it's the Voltage readings. Box 1002 reads on average 1.7 Volts highers that what is reported by ground truth. Figure~\ref{fig:gt_adj_post_1} shows the ground truth data adjusted to be 1.7 Volts higher.
...
I'm generally pretty happy with the Event ground truth comparison. The results show that although we may not be reading exactly what the ground truth sensors are reading, when adjusted for differences in low level metrics, expected Mauka Events match that of the ground truth data.
...
Voltage Incidents include Voltage sags, swells and interruptions as well as Semi-F47 and ITIC classifications. All Voltage Incidents rely on the duration of the Incident in order to provide an accurate classification. Duration information is not provided by ground truth data. Instead, I use thresholding to determine where OPQ Inicdents might have been observed. It's important to note here that Incidents observed by the ground truth data may not meet the duration thresholds to be classfied by OPQ. I attempt to motivate this point using data from Events.
...
These results are not too surprising. Similar to the Voltage Incidents, we observe that OPQ Boxes track less THD Incidents than the UHM ground truth meters. This is to be expected due to the fact that the Incidents are only classified when signals are non-nominal for a specified durations. It's likely (yet unknown due to lack of detailed ground truth data) that the THD Incidents that were observed by the ground truth sensors did not meet the duration requirements for classification.
...
Mauka provides many other Incidents that rely on much more than just simple thresholds.