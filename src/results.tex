...
Let's first compare the results to the estimated storage requirements without TTL found in Section~\ref{sssec:eval_of_dsn_system_requirements}. This might feel a bit contrived, but the purpose of these results is to show how OPQ compares to a similar system that would collect everything.
...
We can see that over the same time period, AML with TTL saved us about 14.6 GB worth of data versus a store everything approach.
...
Here we can see that the actual size of the IL tracks pretty closely to the estimated maximum bounds. This is expected as the IL has a TLL of one year. Since we only have actual data for two and a half months, none of the generated IL have been garbage collected. Since I am comparing the actual data to estimated data without TTL, I expect these two values to trend in a similar fashion. At the end of the data collection period, OPQ collected about 2GB more worth of data than what was projected. This tells me that the estimation parameter for Incident data per second is likely slightly underestimated.
...
Figure shows the actual Laha vs unbounded Laha. Finally, I examine the total size of Laha and compare it to the estimated upper bounds of Laha without TTL. This takes into account all levels within the Laha hierarchy.

Now I will compare the results to the estimated (simulated) storage requirements with TTL. Figure~\ref{fig:actual_laha_vs_unbounded_opq} compares the actual bounds of the entire network to the estimated bounds.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/actual_laha_vs_unbounded_no_iml_opq.png}
    \caption{Actual Laha vs Unbounded Laha for OPQ (No IML)}
    \label{fig:actual_laha_vs_unbounded_no_iml_opq}
\end{figure}

Over a time period of 2 and a half months, the OPQ network saved about 20GB of data as compared to a store everything approach (not including the IML which saved about 2.5 TB of data).

Next I will compare the results gathered from the OPQ deployment to the estimated bounds found in Section~\ref{sssec:evaluation_of_ttl}. Similarly to the previous comparisons, I will offset the actual data to ``force" the data to start from zero.

Figure~\ref{fig:actual_iml_vs_sim_opq} shows the actual IML vs estimated IML.
...
Figure~\ref{fig:actual_aml_vs_sim_opq} shows the actual AML vs estimated AML.
...
Figure~\ref{fig:actual_dl_vs_sim_opq} shows the actual DL vs estimated DL.
...
Figure~\ref{fig:actual_il_vs_sim_opq} shows the actual IL vs estimated IL.
