...
Annotation Phenomena were added to both the OPQ and Lokahi networks.
...
Figure~\ref{fig:lokahi_actual_iml_vs_est} shows the actual IML growth versus the estimated growth.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/lokahi_actual_iml_vs_est.png}
    \caption{IML Growth: Actual vs. Estimated}
    \label{fig:lokahi_actual_iml_vs_est}
\end{figure}

The actual data growth for the IML concludes with about 1 TB less data compared to the estimated growth. This is caused by the fact that the estimated growth uses the mean number of active sensors rather than the actual number of active sensors sending data.

Next I will compare the IML data growth to the simulated data growth. It should be noted that the simulated data includes TTL values, however, Lokahi does not make use of TTL due to network requirements. This provides interesting insights on how the IML data could be bounded within Lokahi assuming TTL were used.
...
I would like to do a direct study on how intermittent renewable energy sources affect PQ on the grid. The Hawaiian islands are a perfect testbed for this and the Laha framework is capable of providing insights into this issue.
