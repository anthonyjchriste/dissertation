...
\begin{align}
	\delta N_{SEN} &= \frac{\sigma N_{SEN}}{\sqrt{T}} \\
	\delta S_{IML} &= \delta N_{SEN} * |S_{SAMP} * SR * T|
\end{align}
...
Similar to what was done for the IML, we can calculate estimate bounds assuming the number of sensors sending data is not constant.

\begin{align}
	\mu S_{SL} &= S_{V} * SR * T * \mu B \pm \delta S_{SL} \\
	\mu S_{AML} &= \sum_{i=1}^{S_{\mu SL_{n}}} \mu S_{SL_{i}} \pm \delta S_{AML}
\end{align}
...
We can provide estimated bounds on the DL with an estimated data rate $\mu DR$ which provides the mean number of bytes generated per second within the DL\@. It should be noted that this includes the parameters for all sub-detections as well.
...
\begin{align}
	\delta DR = \frac{\sigma DR}{\sqrt{T}} \\
	\delta S_{DL} = \delta {DR} * |T|
\end{align}
...
We can provide estimated bounds on the IL assuming a mean incident rate in bytes per second $\mu IR$.
...
\begin{align}
	\delta IR &= \frac{\sigma IR}{\sqrt{T}} \\
	\delta S_{IL} = \delta IR * |T|
\end{align}
