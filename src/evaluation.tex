...
Let us consider the theoretical bounds on storage requirements at each level in the Laha hierarchy.
...
If the sampling rate is not constant, then we can calculate the sensor IML size statistically assuming that we have a known average sample rate $\mu SR$ and a measure of standard deviation $\sigma SR$.
...
Finally, we can calculate the bounds for the IML $\mu S_{IML}$ with a mean sample rate as shown in equation~\ref{iml:uDSN}.

\begin{align}
	\delta S_{IML} &= \sqrt{\sum_{i=1}^{\delta S_{SEN_{n}}} (\delta S_{SEN_{i}})^2} \label{iml:dDSN} \\
	\mu S_{IML} &= (\sum_{i=1}^{B_{n}} \mu S_{SEN_{i}}) \pm \delta S_{IML} \label{iml:uDSN}
\end{align}
...
Equation~\ref{eq:dl:msd} shows the mean detection size $\mu S{D}$.
...
\begin{align}
	...
	\delta S_{SD} &= |\mu S_{SD}| * \sqrt{(\frac{\sigma S_{SAMP}}{\mu S_{SAMP}})^2 + (\frac{\sigma SR}{\mu SR})^2 + (\frac{\sigma SD}{\mu SD})^2} \label{eq:dl5} \\
	\delta S_{D} &= |\mu S_{D}| * \sqrt{(\frac{\sigma S_{SD}}{\mu S_{SD}})^2 + (\frac{\sigma T_{SD}}{\mu T_{SD}})^2} \label{eq:dl6} \\
	\delta S_{DL} &= |\mu S_{DL}| * \sqrt{(\frac{\sigma S_{D}}{\mu S_{D}})^2 + (\frac{\sigma DR}{\mu DR})^2} * |T| \label{eq:dl7}
\end{align}
...
Figure shows the DL growth based on estimated parameters from the OPQ DSN\@.

Table~\ref{table:dl:opq} shows the estimated parameters that were used.
