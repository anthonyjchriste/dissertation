...
In the following sub-sections, I will look at the minimum and maximum bounds for each level in the Laha hierarchy with TTL enabled. The minimumum bounds will always be the data bounded by TTL without any data being saved by events further up the hierarchy. The maximum bounds will be calculated using estimated parameters that quantify the amount of data in each level that is preserved by upper levels.
