...
Equations \Crefrange{iml:SSEN}{iml:SDSN} calculate the maximum bounds for IML storage for individual sensors and the entire DSN respectively when sensor parameters are known for each sensor.

\begin{align}
	&S_{SEN} = S_{SAMP} * SR * T \label{iml:SSEN} \\
	&S_{DSN} = \sum_{i=1}^{B_{n}} S_{SEN_{i}} \label{iml:SDSN}
\end{align}

where:
\begin{align*}
	B &= \text{A sensor in the DSN} \\
	S_{SEN} &= \text{IML sensor size bytes} \\
	S_{DSN} &= \text{IML DSN size bytes} \\
	S_{SAMP} &= \text{Size of sample bytes} \\
	SR &= \text{AML rate Hz} \\
	T &= \text{Sensing time seconds}
\end{align*}

\Crefrange{iml:aSSEN}{iml:aSDSN} calculate minimum and maximum bounds for the IML when the sensor sampling rate and sample size are provided as statistical averages and also calculates the errors on these bounds.
