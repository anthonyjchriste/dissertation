...
Equations \Crefrange{iml:aSSEN}{iml:aSDSN} calculate minimum and maximum bounds for the IML when the sensor sampling rate and sample size are provided as averages.

\begin{align}
	&\mu S_{SEN} = S_{SAMP} * SR * T \label{iml:aSSEN} \\
	&\mu S_{DSN} = \sum_{i=1}^{B_{n}} S_{SEN_{i}} \label{iml:aSDSN}
\end{align}
