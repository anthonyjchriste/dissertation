...
The AML (Aggregate Measurement Level) collects windowed features estimated from the IML data. The OPQ network provides two AMLs, Measurements which are rolled into one second windows and Trends which are rolled into one minute windows. The Lokahi network provides a single AML with windows that vary depending on the sample sensor rate. The previous section evaluated the growth of this level without TTL. This section will add TTL into the evaluation.

The following table displays default TTL values for the OPQ and Lokahi networks.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{Xlll}
		\toprule
		\textbf{Level} & \textbf{OPQ} & \textbf{Lokahi} \\
		\midrule
		$\mu DL_{IML}$ & 94.84 GB & 0.0 \\
		$\mu IL_{IML}$ & 308.96 GB & 0.0 \\
		$\mu PL_{IML}$ & 0.0 & 0.0 \\
		\bottomrule
	\end{tabularx}
	\label{table:ttl_iml}
