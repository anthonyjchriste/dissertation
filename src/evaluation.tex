...
There is a clear upper bound on the size of the DL for a single sensor in the pathological case that the entire data set recorded by a single sensor is one long single Detection. In this pathological case, the upper bounds of the DL is equal to the upper bounds of the IML plus some constant metadata size. The pathological case isn't incredibly useful, so, let's look at this from a statistical standpoint.
...
Figure~\ref{fig:plot_dl_opq_avg} shows the estimated size for the DL on the OPQ network. The errors are small enough that they are not discernible in this plot. Over a period of one year, the DL is expected to grow to 412 gigabytes $\pm$ 1.13 gigabytes.
...
There is a clear upper bound on the size of the IL for a single sensor in the pathological case that the entire data set recorded by a single sensor is one long single Incident. In this pathological case, the upper bounds of the IL is equal to the upper bounds of the IML plus some constant metadata size. The pathological case isn't incredibly useful, so, let's look at this from a statistical standpoint.
...
Let's next compare the estimated sizes of the IL for the OPQ and Lokahi networks using statistics gathered for these networks. Table~\ref{table:estimated_mu_ir} provides the parameters used for these comparisons.
...
Figure~\ref{fig:plot_il_opq_avg} shows the estimated size for the DL on the OPQ network. The errors are small enough that they are not discernible in this plot. Over a period of one year, the IL is expected to grow to 1.55 gigabytes $\pm$ 11 megabytes.
...
We see that the size of the AML also takes up the next largest percentage of space. This is mainly due to the fact that OPQ collects aggregate measurements once per second. Left unbounded, this collection can become quite sizable. 
...
But do these results make sense? They certainly do!
