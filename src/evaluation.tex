...
% TODO Fix AML constraints for Lokahi and regenerate plots at a size of 2546 bytes, also remove stuff about AML including raw data because that isn't really true
% TODO look at the summaries and make sure they still make sense with the most recent computed numbers
% TODO change upper bounds to estimated bounds and add an actual upper bounds section that calculates the pathlological case of all the data being saved
% TODO either convert everything to be per network or per device but not both
\begin{table}[H]
	\centering
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llllll}
		\toprule
		\textbf{Network} & $\bm{\mu DR}$ & $\bm{\sigma DR}$ & \textbf{Data/Day} & \textbf{Data/Week} & \textbf{Data/Year} \\
		\midrule
		OPQ & 8211.7 & 185544.8 & 0.71 GB & 4.97 GB & 258.96 GB \\
		Lokahi & 406.48 & 2199.26 & 25.11 MB & 245.84 & 12.82 GB \\
		\bottomrule
	\end{tabularx}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llllll}
		...
		\textbf{Network} & $\bm{\mu DR}$ & $\bm{\sigma DR}$ & \textbf{Data/Day} & \textbf{Data/Week} & \textbf{Data/Year} \\
		\midrule
		OPQ & 438.58 & 6288.48 & 0.04 GB & 0.27 GB & 13.83 GB \\
		Lokahi & 37.12 & 12004.18 & 3.21 MB & 22.45 MB & 1.17 GB \\
		\bottomrule
	\end{tabularx}
...
If we don't take into account saving IML data from higher levels, calculating lower bounds of the IML for a network can be accomplished by simply bounding its growth by its TTL\@. By substituting $T$ in Equations~\ref{iml:SSEN} and~\ref{eq:iml:mu_size_iml} with the the number of seconds in the IML TTL (15 minutes), we can calculate the minimum bounds of the IML for a single sensor as shown in Table~\ref{table:ttl_a_iml}.
...
However, data is not only just discarded when it reaches its TTL, but it can have its TTL adjusted when that data is used in identifying a feature of a higher Laha level. When a higher (DL, IL, or PL) uses data from the IML, that data will be adjusted to live for as long as the highest level that it was used in. This must be taken into consideration when calculating the bounds of the IML. To find these numbers, I've taken data directly from the OPQ and Lokahi deployments to determine on average how often IML data is saved.

Equation~\ref{eq:s_iml_ttl_full} provides the calculation for obtaining the estimated bounds of the IML where $\mu DL_{IML}$ is the average amount of IML data saved by the DL within its TTL of 1 month, $\mu IL_{IML}$ is the average amount of IML data saved by the IL within its TTL of 1 year, and $\mu PL_{IML}$ is the average amount of IML data saved by the PL within its TTL of 2 years.

\begin{equation}\label{eq:s_iml_ttl_full}
	S_{IML\_TLL} = \mu S_{IML} + \mu DL_{IML} + \mu IL_{IML} + \mu PL_{IML}
\end{equation}

Where $S_{IML}$ is calculated from~\ref{eq:iml:mu_size_iml} and $T$ is substituted with the number of seconds in the IML TTL (15 minutes).

It should be noted that IML data within OPQ is only preserved when Detections, Incidents, or Phenomena that use that data are created. Further, the IML data lives on the OPQ Boxes themselves, so preserving IML data isn't as simple as just updating its TTL, but rather IML data is copied into Detections, Incidents, and Phenomena and is garbage collected when those Detection, Incidents, or Phenomena are garbage collected.

The Lokahi network works slightly differently and the IML data is collected from all sensors and stored in the database allowing the IML data in Lokahi to have its TTL updated directly rather than copied into the system like OPQ\@.

Table~\ref{table:ttl_iml_params} provides the estimated parameters for the OPQ and Lokahi networks required for determining the estimated IML bounds with TTL. These parameters are the estimated amount of data saved from the IML by higher levels in the hierarchy (DL, IL, and PL). This data was estimated from the OPQ and Lokahi deployment by taking the ratio of data received over the course of the deployment and adjusting it to the duration of the particular level's TTL\@.

\begin{table}[H]
	\centering
	\caption{Estimated IML Data Saved by Level}
	\begin{tabularx}{\textwidth}{Xllll}
		\toprule
		\textbf{Network} & \boldmath{$\mu DL_{IML}$} & \boldmath{$\mu IL_{IML}$} & \boldmath{$\mu PL_{IML}$} & \boldmath{$\mu N$} \\
		\midrule
		OPQ & 94.83 GB & 308.93 GB & n/a & 15 \\
		Lokahi & 0.98 GB & 1.10 GB & n/a & 60 \\
		\bottomrule
	\end{tabularx}
	\label{table:ttl_iml_params}
\end{table}

Table~\ref{table:ttl_iml_est} provides the estimated bounds of the IML layer for the OPQ and Lokahi networks using estimated parameters from deployments of those networks.

\begin{table}[H]
	\centering
	\caption{Estimated IML Bounds}
	\begin{tabularx}{\textwidth}{Xl}
		\toprule
		\textbf{Network} & \textbf{Estimated IML Bounds} \\
		\midrule
		OPQ & 404.08 GB \\
		Lokahi & 3.81 GB \\
		\bottomrule
	\end{tabularx}
	\label{table:ttl_iml_est}
\end{table}

Finally, the upper bounds of the IML level can be calculated by examining a couple of pathological cases. The first case is where all data is saved by Phenomena that do not expire. In this case, the IML data data will remain unbounded because all IML data is referenced by Phenomena that does not expire. This extreme case is unlikely to happen in practice.

The other more likely (yet still rare) pathological case is that all IML data is saved by Phenomena that expires within the default Phenomena TTL of two years. In this case, the IML data will take two years to expire and the upper bounds of the IML with TTL can be found by substituting the number of seconds in two years for $T$ is Equation~\ref{eq:iml:mu_size_iml}. The estimated upper bound are provided in Table~\ref{table:ttl_iml_max}.

\begin{table}[H]
	\centering
	\caption{Estimated IML Upper Bounds}
	\begin{tabularx}{\textwidth}{Xll}
		\toprule
		\textbf{Network} & \textbf{Max IML Bounds/Device} & \textbf{Max IML Bounds/Network} \\
		\midrule
		OPQ & & 22.72 TB \\
		Lokahi & & 121.18 TB \\
		\bottomrule
	\end{tabularx}
	\label{table:ttl_iml_max}
\end{table}

\paragraph{Evaluation of AML with TTL}
The AML (Aggregate Measurement Level) collects windowed features estimated from the IML data. The OPQ network provides two AMLs, Measurements which are rolled into one second windows and Trends which are rolled into one minute windows. The Lokahi network provides a single AML with windows that vary depending on the sensor sample rate. The previous section evaluated the growth of this level without TTL. This section will add TTL into the evaluation.

Table~\ref{table:ttls_aml} displays default TTL values for the OPQ and Lokahi networks.
...
The minimum bounds of the AML with TTL can be found by using Equations~\ref{eq:aml:mu_s_sl} and~\ref{eq:aml:mu_s_aml} and substituting $T$ for the AML's TTL.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{Xll}
		\toprule
		\textbf{Network} & \textbf{Min AML Bounds Single Device} & \textbf{Min AML Bounds Network} \\
		\midrule
		OPQ (Measurements) & 12.53 MB & 187.92 MB \\
		OPQ (Trends) & 6.55 MB & 98.28 MB \\
		OPQ (Total) & 18.58 MB & 286.2 MB \\
		Lokahi & 60.15 MB & 3.61 GB \\
		\bottomrule
	\end{tabularx}
...
Calculation of the upper bounds can be found by Equation~\ref{eq:s_aml_ttl_full}.

\begin{equation}\label{eq:s_aml_ttl_full}
	S_{AML\_TLL} = \mu S_{AML} + \mu DL_{AML} + \mu IL_{AML} + \mu PL_{AML}
\end{equation}
...
Table~\ref{table:ttls_aml_params} provides the estimated AML saved data parameters obtained from the OPQ and Lokahi deployments. These parameters were found by examining the mean duration of Detections, Incidents, and Phenomena within their TTL bin and by also taking into consideration the highest level within Laha that the AML data reached.

\begin{table}[H]
	\centering
	\caption{Estimated Parameters for Calculating AML Data Saved per Level}
	\begin{tabularx}{\textwidth}{Xllll}
		\toprule
		\textbf{Description} & \textbf{Sample Size} & \textbf{Duration S/TTL} & \textbf{Rate} & \textbf{Saved Size} \\
		\midrule
		OPQ DL (Measurements) 	& 145 	& 1968.40 s/mo 		& 1 Hz 					& 0.29 MB 	\\
		OPQ DL (Trends) 		& 325 	& 1968.40 s/mo 		& $\frac{1}{60}$ Hz 	& 0.01 MB 	\\
		OPQ DL (Total)			& 	  	&                 	&                   	& 0.30 MB 	\\
		OPQ IL (Measurements) 	& 145 	& 206624.08 s/yr 	& 1 Hz 					& 29.96 MB 	\\
		OPQ IL (Trends) 		& 325 	& 206624.08 s/yr	& $\frac{1}{60}$ Hz 	& 1.12 MB 	\\
		OPQ IL (Total)			&     	&                 	&                   	& 31.08 MB 	\\
		OPQ PL (Measurements) 	& 145 	&                	& 1 Hz 					& 		 	\\
		OPQ PL (Trends) 		& 325 	&               	& $\frac{1}{60}$ Hz 	& 		 	\\
		OPQ PL (Total)			&     	&                 	&                   	& 		 	\\
		OPQ (Total)				&     	&                 	&                   	& 32.2 MB 	\\
		Lokahi DL	            & 2546  & 152310.38 s/mo	& $\frac{1}{40.96}$ Hz 	& 9.47 MB	\\
		Lokahi IL	            & 2546  & 79098.60 s/yr 	& $\frac{1}{40.96}$ Hz 	& 4.92 MB 	\\
		Lokahi PL	            &       &               	&                      	& 		 	\\
		Lokahi (Total)          &       &               	&                      	& 14.29 MB 	\\
		\bottomrule
	\end{tabularx}
	\label{table:ttls_aml_params}
\end{table}

Plugging the above values into Equation~\ref{eq:s_aml_ttl_full} allows us to find the estimated size of the AML with TTL using parameters from the deployed OPQ and Lokahi networks. These results are shown in Table

\begin{align}
	\mu S_{SL}OPQ\_Measurements &= S_{V} * SR * T * \mu B 										\\
	                            &= 145 * \frac{1}{1} * 86400 * 15 								\\
								&= 187.92 MB 													\\
	\mu S_{SL}OPQ\_Trends       &= S_{V} * SR * T * \mu B  										\\
								&= 325 * \frac{1}{60} * 1209600 * 15 							\\
								&= 98.28 MB 													\\
	\mu S_{AML}OPQ 				&= \mu S_{SL}OPQ\_Measurements + \mu S_{SL}OPQ\_Trends  		\\
								&= 286.2 MB 													\\
	\mu S_{AML\_TLL}OPQ 		&= \mu S_{AML} + \mu DL_{AML} + \mu IL_{AML} + \mu PL_{AML}  	\\
								&= 286.2 + 0.30 + 31.08											\\
								&= 317.58 MB													\\
	\mu S_{AML}Lokahi  			&= S_{V} * SR * T * \mu B 				    					\\
								&= 2546 * \frac{1}{40.96} * 1209600 * 60 						\\
								&= 4.51 GB														\\
	\mu S_{AML\_TLL}Lokahi 		&= \mu S_{AML} + \mu DL_{AML} + \mu IL_{AML} + \mu PL_{AML}  	\\
								&= 4.51GB + 9.47MB + 4.92MB										\\
								&= 4.52 GB
\end{align}

\begin{table}[H]
	\centering
	\caption{Estimated Size AML}
	\begin{tabularx}{\textwidth}{Xl}
		\toprule
		\textbf{Description} & \textbf{Size} \\
		\midrule
		OPQ (Measurements) 	& 5.29 GB   \\
		OPQ (Trends) 		& 2.56 GB \\
		OPQ (Total) 		& 7.85 GB   \\
		Lokahi & \\
		\bottomrule
	\end{tabularx}
	\label{table:ttls_aml_est}
\end{table}
