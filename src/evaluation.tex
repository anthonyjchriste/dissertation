...
If we don't take into account saving IML data from higher levels, calculating upper bounds of the IML for a single device can be accomplished by simply bounding its growth by its TTL\@. This is provided in equation~\ref{eq:s_iml_ttl}.

\begin{equation}\label{eq:s_iml_ttl}
	IML_{TTL} = SR * S_{SAMP} * TTL_{IML}
\end{equation}

Plugging in values from OPQ and Lokahi nets the following bounds assuming a 15 minute TTL for IML data.
...
Equation~\ref{eq:s_iml_ttl_full} provides the calculation for obtaining the full estimated bounds of the IML where $\mu DL_{IML}$ is the average amount of IML data saved by the DL within its TTL of 1 month, $\mu IL_{IML}$ is the average amount of IML data saved by the IL within its TTL of 1 year, and $\mu PL_{IML}$ is the average amount of IML data saved by the PL within its TTL of 2 years.

\begin{equation}\label{eq:s_iml_ttl_full}
	IML_{TTL} = SR * S_{SAMP} * TTL_{IML} + \mu DL_{IML} + \mu IL_{IML} + \mu PL_{IML}
\end{equation}

Table~\ref{table:ttl_iml_params} provides the estimated parameters for the OPQ and Lokahi networks required for determining the IML bounds with TTL.
...
Table provides the maximum bounds of the IML layer for the OPQ and Lokahi networks using estimated parameters from deployments of those networks.
