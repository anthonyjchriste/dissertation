...
Next, we can calculate the IML size for the entire network by summing up the IML size for each individual sensor $B$ in the network. Equation~\ref{iml:DSEN} provides these calculations.
...
The error for sampling rate $\delta_{\mu SR}$ is given in equation~\ref{iml:uErr}. The error calculation for a single sensor is given in equation~\ref{iml:uSSEN}.

By propagating the error as an error multiplied with a constant, we get a final bounds for a single sensor $\mu S_{SEN}$ as described in equation~\ref{iml:sbounds}.

\begin{align}
	\delta_{\mu SR} &= \frac{\sigma}{\sqrt{N}} \label{iml:uErr} \\
...
Each sample in this level is generally stored using build in machine types that generally take 1, 2, 4, or 8 bytes. Further, depending on the network requirements, each sample may or may not have an associated timestamp. Each timestamp generally adds between 4 and 8 bytes to each sample.

First, let's examine how sample size affects constant data pressure with a stable sampling rate

%\Crefrange{iml:aSSEN}{iml:aSDSN} calculates minimum and maximum bounds for the IML when the sensor sampling rate and sample size are provided as statistical averages and also calculates the errors on these bounds.
%
%\begin{align}
%	&\mu S_{SEN} = \mu S_{SAMP} * \mu SR * T \label{iml:aSSEN} \\
%	&\mu S_{SENERR} =
%	&\mu S_{DSN} = \sum_{i=1}^{B_{n}} \mu S_{SEN_{i}} \label{iml:aSDSN} \\
%	&\mu B_{L} = something
%	&\mu B_{U} = something
%\end{align}
%
%where:
%\begin{align*}
%	B &= \text{A sensor in the DSN} \\
%	\mu S_{SEN} &= \text{IML sensor size bytes} \\
%	\mu S_{DSN} &= \text{IML DSN size bytes} \\
%	\mu S_{SAMP} &= \text{Size of sample bytes} \\
%	\mu SR &= \text{AML rate Hz} \\
%	T &= \text{Sensing time seconds}
%\end{align*}
...
Laha allows for multiple sub-levels within the AML level. For instance, the OPQ DSN has both measurements and trends AMLs which summarize data at different rates. We need to take this into considering when generation upper bounds for the AML level.
...
\begin{align*}
	B &= \text{A sensor in the DSN} \\
	S_{V} &= \text{Average (or constant) size of AML value in bytes} \\
	S_{SL} &= \text{AML sub-level size bytes} \\
	S_{AML} &= \text{AML size bytes} \\
	SR &= \text{Sensor sample rate Hz} \\
	T &= \text{Sensing time seconds}
\end{align*}
...
Calculating the bounds of this collection is further complicated by the fact that Detections are not generated at a constant rate and the window length of the detections can be highly variable. Therefore, I provide statistical calculations for these bounds.

\begin{align}
	&S_{SUBDET} = S_{SAMP} * SR * T + S_{SUBMETA} \\
	&S_{DET} = \sum_{i=1}^{S_{SUBDEC_{n}}} S_{SUBDEC_{i}} \\
	&S_{DL} =
\end{align}

where:
\begin{align*}
	S_{SUBDET} &= \text{Size of individual sub-detection in bytes} \\
	S_{SUBMETA} &= \text{Size of metadata for sub-detection in bytes} \\
	S_{SAMP} &= \text{Size of sample bytes} \\
	SR &= \text{Sensor sampling rate Hz} \\
	S_{META} &= \text{Size of a detection in bytes} \\
	S_{DET} &= \text{Size of individual sub-detection in bytes} \\
	\mu_{SR} &= \text{Mean detection rate Hz} \\
	T &= \text{Detection window length seconds}
\end{align*}
